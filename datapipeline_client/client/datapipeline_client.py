...
'''
This DataPipelineClient class interfaces with Hyperledger Sawtooth through the REST API
...
class BaseClientClass(object):
    ...
    def _wrap_and_send(self, action, *values):
        ...
        for val in values:
            rawPayload = ",".join(list(rawPayload, str(val)))

        payload = rawPayload.encode()

        # Construct the address where we'll store our state
        address = self._address
        inputAddressList = list(address)
        outputAddressList = list(address)
        ...
        header = BatchHeader(
            signer_public_key=self._publicKey,
            transaction_ids=list(txn.header_signature for txn in transactionList)
        ).SerializeToString()
        ...
        batch_list = BatchList(batches=list(batch))
    ...
    def data_transfer(self, json_file, clientToKey, accept):
        if (accept):
            try:
                ...
                raise Exception("Encountered an Error during Data Transfer", err)
